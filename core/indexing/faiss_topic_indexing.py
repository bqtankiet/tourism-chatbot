...
class FaissTopicIndexer:
    ...
    def build(
        ...
        vectors_path = output_dir / "topic_vectors.npy"
    ...
    def search(index_dir: str | Path, query: str, top_k: int = 5) -> list[dict]:
        ...
        if not manifest_path.exists():
            raise ValueError(f"manifest.json not found in: {index_dir}")
        ...
        with open(index_dir / "topic_metadata.json", "r", encoding="utf-8") as f:
...
if __name__ == "__main__":
    ...
    indexer = FaissTopicIndexer(project_root=root, model_name="BAAI/bge-m3")
